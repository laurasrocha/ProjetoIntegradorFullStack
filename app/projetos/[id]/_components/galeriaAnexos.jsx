...
export default function GaleriaAnexos({ fotos, open, onClose }) {
  if (!open) return null;

  const [imgFull, setImgFull] = useState(null);
  const [imgIndex, setImgIndex] = useState(0);

  const arquivos = fotos ? fotos.split(",") : [];

  // Apenas imagens (png, jpg, jpeg, webp)
  const imagens = arquivos.filter((file) => {
    ...
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-2xl overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="text-red-500 font-semibold hover:cursor-pointer" onClick={onClose}>
            Fechar
          </button>
        </div>
                ...
                {!isPdf ? (
                  <div className="relative group cursor-pointer">
                    <img
                      ...
                      onClick={() => {
                        // índice CORRETO dentro do array imagens
                        const imgIdx = imagens.indexOf(file);
                        setImgIndex(imgIdx);
                        setImgFull(file);
                      }}
                    />
